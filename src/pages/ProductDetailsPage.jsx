...
export default function ProductDetailsPage() {
        ...
        if (result && result.similarProducts) {
          ...
          const allSimilarProducts = await Promise.all(
            similarProductIds.map((sid) =>
              fetchClothById(sid, undefined, setIsError),
            ),
          )
  ...
  useEffect(() => {
    if (cbBought2Ref.current) cbBought2Ref.current.checked = false
    if (cbBought3Ref.current) cbBought3Ref.current.checked = false
  }, [id])
      ...
      timerId = setInterval(async () => {
        try {
          ...
          if (hours === 23 && minutes === 59 && seconds === 59) {
            ...
            setUpdated(true)
            const cloth = await fetchClothById(id, undefined, setIsError)
  ...
  async function updateCreateOrder(targetId, data) {
    try {
      ...
      if (response.length) {
        ...
        await saveCreateOrder(data, undefined, setIsError)
      }
      setUpdated(true)
    } catch (error) {
      throw error
  ...
  async function addToCart(e) {
    try {
      ...
      const cloth = await fetchClothById(currentTargetId, undefined, setIsError)
      if (cloth) {
  ...
  if (loading || !product || !similarProducts.length) {
        ...
        >
          ...
          >
            ...
            <div className="me-md-5">
              ...
              <p className={`fw-bold lh-sm ${styles.productDescription} mb-1`}>
          ...
          <section className={`${styles.productSpecsSection}`}>
            ...
            >
              ...
              >
                ...
                <table className={`${showTable1 ? `${styles.showTable}` : ""}`}>
                  <tbody>
              ...
              >
                ...
                <table className={`${showTable2 ? `${styles.showTable}` : ""}`}>
                  <tbody>
                    {itemDetailsKeys &&
            ...
            >
              ...
              >
                ...
                <table className={`${showTable3 ? `${styles.showTable}` : ""}`}>
                  <tbody>
                    {styleKeys &&
                      styleKeys.map((key) => {
                        return (
              ...
              >
                ...
                <table className={`${showTable4 ? `${styles.showTable}` : ""}`}>
                  <tbody>
                    {topHighlightsKeys &&
