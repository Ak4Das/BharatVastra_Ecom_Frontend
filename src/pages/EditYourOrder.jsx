...
export default function EditYourOrder() {
  ...
  const address =
    Object.keys(user).length && user.address.find((address) => address.selected)
  ...
  async function save(e) {
    try {
      ...
      order && order.item.length === 0
        ? await deleteOrderById({ id: order.id, setIsError, navigate })
        : await updateOrder({
            id: order.id,
            data: order,
            setIsError,
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const order = await fetchOrderByOrderId({
          orderId,
